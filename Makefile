...
e2e-local: .e2e-server-build
	E2E_BINARY=target/debug/cf-gears-example-server $(PYTHON) tools/scripts/ci.py e2e-local -- $(GEAR_E2E_SCOPE)
...
.PHONY: all dist check ci ci_test ci_docs build build-debug .cargo-build .split-debug quickstart example mini-chat mini-chat-docker mini-chat-helm mini-chat-helm-template mini-chat-up mini-chat-down mini-chat-port-forward
