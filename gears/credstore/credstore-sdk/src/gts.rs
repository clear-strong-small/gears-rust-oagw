...
//! against the registry and the PDP compiles a tenant-scoped grant into
//! tenant constraints on `owner_tenant_id`. The same string is the
...
/// compiles a tenant-scoped grant into tenant constraints on
