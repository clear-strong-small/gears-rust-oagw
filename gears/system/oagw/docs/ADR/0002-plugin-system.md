...
- `OAuth2ClientCredAuthPlugin`: OAuth2 client credentials flow (registered twice — `Form` and `Basic` client-auth-method variants — see [ADR: OAuth2 Client Credentials Auth Plugin](./0008-oauth2-client-credentials-auth-plugin.md))
...
Request timeout and CORS are core Data Plane logic, not `GuardPlugin` trait implementations in `infra/plugin/` — see [ADR: CORS](./0004-cors.md). Their GTS identifiers (`cf.core.oagw.timeout.v1`, `cf.core.oagw.cors.v1`) exist for types-registry cataloging only and are not resolvable via `GuardPluginRegistry`.
...
Separate ToolKit gears implementing the plugin traits:
...
Code review confirms: the `AuthPlugin`, `GuardPlugin`, and `TransformPlugin` traits are defined. Built-in implementations in `infra/plugin/`. External plugins register via ToolKit dependency injection.
