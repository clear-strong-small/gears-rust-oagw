...
In the persistence model this corresponds to `oagw_route.match_type` selecting the match key table (`oagw_route_http_match` vs `oagw_route_grpc_match`).
...
- [ADR: Control Plane Caching](./0005-data-plane-caching.md) — L1 cache mitigates DP→CP dependency
